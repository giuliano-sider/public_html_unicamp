...
#macro make_cactus_thorn(altura, raio) //a altura e o raio sao do cacto mesmo
  cone{
    <0, (altura-raio)*rand(seedHeight), 0>, altura/100.0,
    <raio+rand(seedLength)*raio, 0, 0> , 0.01
  
    pigment{thorn_color}
    rotate <0,seedAngle*360,0>
  }
#end
...
#macro make_cactus(profundidade, num_espinhos, altura, raio, num_sub_cactos)
  ...
  #declare cactus_body = make_cactus_body(altura, raio);
  #local i = 0;
  ...
  union{
    ...
    #for(i, 0, num_espinhos)
      ...
      #for(i, 0, num_sub_cactos)
        object{
          make_cactus(profundidade-1, num_espinhos/2, altura/2, raio/2, num_sub_cactos)
          rotate <360*rand(seedAngle), 360*rand(seedAngle), 360*rand(seedAngle)>
          translate <0,altura + (altura/2)*rand(seedHeight),0>
        }
        #local i = i + 1;
      #end
...
object{
  make_cactus(2, 10, 10, 1, 3) //profundidade, num_espinhos, altura, raio, num_sub_cactos
}
